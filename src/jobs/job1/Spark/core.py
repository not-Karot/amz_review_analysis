...
sc = SparkContext(appName="Amazon Review Analysis - Job 1")
...
data = data.map(lambda line: line.split("\t")).map(
    lambda fields: (fields[0], int(fields[1]), clean_text(fields[2]))
)
...
word_counts = data.flatMap(
    lambda x: [((x[0], x[1], word), 1) for word in x[2] if len(word) >= 4]
...
top_products = (
    review_counts.sortBy(lambda x: (-x[1], x[0]))
    .groupBy(lambda x: x[0][0])
    .map(lambda x: (x[0], sorted(list(x[1]), key=lambda y: y[1], reverse=True)[:10]))
)


top_products = top_products.flatMap(
    lambda x: [((x[0], product[0][1]), product[1]) for product in x[1]]
)

# Prendi le parole solo per i top prodotti
top_words = (
    word_counts.map(lambda x: ((x[0][0], x[0][1]), (x[0][2], x[1])))
    .join(top_products)
...
top_words = (
    top_words.sortBy(lambda x: (-x[1][1], x[0]))
    .groupBy(lambda x: x[0])
    .map(lambda x: (x[0], sorted(list(x[1]), key=lambda y: y[1][1], reverse=True)[:5]))
)
